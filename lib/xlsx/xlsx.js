...
class XLSX {
  ...
  _processNodeJsEntry(entry, stream) {
    return new Promise((resolve, reject) => {
      const reader = entry.nodeStream();
      ...
      reader.on('data', chunk => {
        ...
        while (content.length >= chunkSize) {
          stream.write(content.substring(0, chunkSize));
      ...
      if (!entry.dir) {
        ...
        if (
          ...
          if (process.browser) {
            ...
            const content = bufferToString(await entry.async('nodebuffer'));
            const chunkSize = 16 * 1024;
            for (let i = 0; i < content.length; i += chunkSize) {
              stream.write(content.substring(i, i + chunkSize));
            }
          } else {
            await this._processNodeJsEntry(entry, stream);
          }
        }
        stream.end();
        switch (entryName) {
  ...
  async addWorksheets(zip, model) {
    ...
    model.worksheets.forEach(worksheet => {
      ...
      zip.appendXMLParts(xmlStream.xmlArray, {
